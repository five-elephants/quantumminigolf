...
unsigned char* V4Linux2Camera::getFrame()  {
    ...
    if(colour) {
	if (pixelformat==V4L2_PIX_FMT_YUYV) yuyv2rgb(width, height, cam_buffer, buffer);
        else if (pixelformat==V4L2_PIX_FMT_UYVY) uyvy2rgb(width, height, cam_buffer, buffer);
        //else if (pixelformat==V4L2_PIX_FMT_YUV420) memcpy(buffer,cam_buffer,width*height);
	else if (pixelformat==V4L2_PIX_FMT_YUV420) ccvt_420p_bgr24(width, height, cam_buffer, buffer);
	else {
		assert(false);
		 return NULL;
	}
