...
#ifndef NDEBUG
void PortVideoSDL::saveBuffer(unsigned char* buffer, int depth) {
	...
	if (depth==1) {
		...
		char fileName[256];
#ifdef WIN32
		sprintf(fileName,".\\recording\\%s%ld.ppm",zero,framenumber_);
#else
		sprintf(fileName,"./recording/%s%ld.ppm",zero,framenumber_);
#endif
		FILE*  imagefile=fopen(fileName, "w");
		fprintf(imagefile,"P6\n%u %u 255\n", width_, height_);
		fwrite((const char *)buffer, 1,  width_*height_*depth, imagefile);
		fclose(imagefile);		
	}
}
#endif
...
void PortVideoSDL::frameStatistics(long cameraTime, long processingTime, long totalTime) {
	...
	if (diffTime >= 1) {
		...
		frames_ = 0;
		cameraTime_ = processingTime_ = totalTime_ = 0;
...
bool PortVideoSDL::initWindow() {

	int videoFlags = SDL_HWSURFACE;
	if( fullscreen_ ) videoFlags |= SDL_FULLSCREEN;
	
	window_ = SDL_SetVideoMode(width_, height_, 32, videoFlags);
	if ( window_ == NULL ) {
    ...
    while( SDL_PollEvent( &event ) ) {
		...
		case SDL_KEYDOWN:
			...
			if( event.key.keysym.sym == SDLK_n ){
				...
				if (stat("./recording",&info)!=0) mkdir("./recording",0777);
#endif
				if (displayMode_==DEST_DISPLAY) saveBuffer(destBuffer_,bytesPerDestPixel_);
				else saveBuffer(sourceBuffer_,bytesPerSourcePixel_);
			} 
#endif
...
void PortVideoSDL::allocateBuffers()
{
	...
	if (sourceDepth_==8) SDL_SetPalette(sourceImage_, SDL_LOGPAL|SDL_PHYSPAL, palette_, 0, 256 );
	//else SDL_SetColorKey(sourceImage_, SDL_SRCCOLORKEY, SDL_MapRGB(sourceImage_->format, 0, 0, 0));
	...
	if (destDepth_==8) SDL_SetPalette(destImage_, SDL_LOGPAL|SDL_PHYSPAL, palette_, 0, 256 );
	//else SDL_SetColorKey(destImage_, SDL_SRCCOLORKEY, SDL_MapRGB(destImage_->format, 0, 0, 0));
...
PortVideoSDL::PortVideoSDL(const char* name, const char* cfg)
	: error_( false )
	, pause_( false )
	, help_( false )
	, framenumber_( 0 )
